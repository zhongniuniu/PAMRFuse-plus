...
class DeformableNet(nn.Module):
    ...
    def forward(self, tgt, src, real,shape=None): #tgt: fake MRI; src: real MRI; real: real PAT;
        ...
        c31 = self.enc[2](self.enc[1](self.enc[0](real))) # torch.Size([16, 16, 128, 128])
        ...
        c32 = self.enc[5](self.enc[4](self.enc[3](c31))) # torch.Size([16, 32, 64, 64])

        ##################### REAL Estimation at scale-2 #######################
        corr2_r = MatchCost(c32, c12)  # torch.Size([16,  1, 64, 64])
        x_r = torch.cat((corr2_r, c32), 1)  # torch.Size([16, 33, 64, 64])
        x_r = self.conv2_0(x_r)  # torch.Size([16, 48, 64, 64])
        x_r = self.conv2_1(x_r)  # torch.Size([16, 32, 64, 64])
        x_r = self.conv2_2(x_r)  # torch.Size([16, 16, 64, 64])
        flow2_r = self.predict_flow2a(x_r)  # torch.Size([16, 2, 64, 64]) flow2: flow field
        upfeat2_r = self.resize(x_r)  # torch.Size([16, 16, 128, 128])

        x_r = self.dc_conv2_0(flow2_r)  # torch.Size([16, 48, 64, 64])
        x_r = self.dc_conv2_1(x_r)  # torch.Size([16, 48, 64, 64])
        x_r = self.dc_conv2_2(x_r)  # torch.Size([16, 32, 64, 64])

        refine_flow2_r = self.predict_flow2b(x_r) + flow2_r  # torch.Size([16, 2, 64, 64])
        int_flow2_r = refine_flow2_r
        # int_flow2 = self.integrate2(refine_flow2)
        up_int_flow2_r = self.resize(int_flow2_r)  # torch.Size([16, 2, 128, 128])
        features_s_warped_r, _ = self.spatial_transform_f(c11, up_int_flow2_r)  # torch.Size([16, 16, 128, 128])

        ##################### SRC Estimation at scale-2 #######################
        ...
        ##################### REAL Estimation at scale-1 #######################
        corr1_r = MatchCost(c31, features_s_warped_r)  # torch.Size([16, 1, 128, 128])
        x_r = torch.cat((corr1_r, c31, up_int_flow2_r, upfeat2_r), 1)  # torch.Size([16, 35, 112, 112])
        x_r = self.conv1_0(x_r)  # torch.Size([16, 48, 128, 128])
        x_r = self.conv1_1(x_r)  # torch.Size([16, 32, 128, 128])
        x_r = self.conv1_2(x_r)  # torch.Size([16, 16, 128, 128])
        flow1_r = self.predict_flow1a(x_r) + up_int_flow2_r  # torch.Size([16, 2, 128, 128])

        x_r = self.dc_conv1_0(flow1_r)  # torch.Size([16, 48, 128, 128])
        x_r = self.dc_conv1_1(x_r)  # torch.Size([16, 48, 128, 128])
        x_r = self.dc_conv1_2(x_r)  # torch.Size([16, 32, 128, 128])
        refine_flow1_r = self.predict_flow1b(x_r) + flow1_r  # torch.Size([16, 2, 128, 128])
        int_flow1_r = refine_flow1_r
        # int_flow1 = self.integrate1(refine_flow1)

        ##################### SRC Estimation at scale-1 #######################
        ...
        ##################### Upsample to scale-0 #######################
        int_flow1 = refine_flow1 + refine_flow1_r
        ...
        # wd+
        f_warp, _ = self.spatial_transform(real, (-flow)) # torch.Size([16, 1, 256, 256]) torch.Size([16, 256, 256, 2])
...
if __name__ == '__main__':
    ...
    a = torch.randn(2, 1, 300, 300).cuda()
    b = torch.randn(2, 1, 300, 300).cuda()
